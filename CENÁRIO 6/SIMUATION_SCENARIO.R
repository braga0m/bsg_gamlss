...
X <- matrix(c(rep(1,VN[1]), x1, x2), ncol=3, byrow=F) #regressor matrix for the median submodel
Z <- matrix(c(rep(1,VN[1]), x1, x2), ncol=3, byrow=F) #regressor matrix for the sigma submodel
W <- matrix(c(rep(1,VN[1]), x1), ncol=2, byrow=F) #regressor matrix for the nu submodel
...
for(l in 1:length(VN)){
  ...
  while(cont < NREP)
  {
    ...
    fit_result <- tryCatch({
      ...
      fitMLE <- gamlss(formula = y~ SampleG$X[,2] + factor(SampleG$X[,3]), 
                       sigma.formula = ~SampleG$Z[,2] + factor(SampleG$Z[,3]), 
                       nu.formula = ~SampleG$W[,2], 
